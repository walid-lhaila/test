...
<body>
    <nav x-data="{ isOpen: false }" class="relative bg-black shadow ">
        <div class="container px-6 h-[60px]  mx-auto md:flex">
            ...
            <div x-cloak :class="[isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full']" class="absolute inset-x-0 w-full px-6 py-2 transition-all duration-300 ease-in-out bg-white dark:bg-black    md:mt-0 md:p-0 md:top-0 md:relative md:opacity-100 md:translate-x-0 md:flex md:items-center md:justify-between">
                <div class="flex flex-col px-2 -mx-4 md:flex-row md:mx-10 md:py-0">
                    <a href="{{ route('proprietaireDashboard') }}" class="px-2.5 py-2 text-gray-700 transition-colors duration-300 transform rounded-lg dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 md:mx-2">Home</a>
                    <a href="{{ route('proprietaireAnnonces') }}" class="px-2.5 py-2 text-gray-700 transition-colors duration-300 transform rounded-lg dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 md:mx-2">Annonces</a>
                </div>
                <div>
                    <a href="{{route('login')}}">
                        <button>
    ...
    <section>
        ...
        <div id="form" class="absolute w-full h-full inset-0 bg-opacity-50 backdrop-filter backdrop-blur-md flex justify-center items-center bg-gray-500 scale-0 duration-300">
            <div class="bg-black w-[560px] ml-[20px]">
                <form action="{{route('annonce.store')}}" method="post" enctype="multipart/form-data" class="max-w-md mx-auto bg-black py-10">
